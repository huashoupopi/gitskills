...
使用git status可以给出状态和命git blame <文件名>

作用：它会把文件的每一行代码旁边都显示出 “是谁、在什么时候、哪一次提交” 修改的。

场景：当你发现一行代码写得很奇怪，想知道是自己什么时候写的，或者这行代码是为了修什么 Bug 写的，用这个命令一目了然。

git clean -fd

作用：强制删除工作区所有**没有被追踪（Untracked）**的文件和文件夹。

场景：当你编译代码生成了一大堆乱七八糟的 .log, .tmp 文件，而你又忘了把它们加到 .gitignore 里时，用这个命令可以一键还你一个干干净净的工作区。令提示
...
- git blame <文件名>
  ...
  场景：当你编译代码生成了一大堆乱七八糟的 .log, .tmp 文件，而你又忘了把它们加到 .gitignore 里时，用这个命令可以一键还你一个干干净净的工作区。git blame <文件名>
  ...
  场景：当你编译代码生成了一大堆乱七八糟的 .log, .tmp 文件，而你又忘了把它们加到 .gitignore 里时，用这个命令可以一键还你一个干干净净的工作区。
...
场景：当你编译代码生成了一大堆乱七八糟的 .log, .tmp 文件，而你又忘了把它们加到 .gitignore 里时，用这个命令可以一键还你一个干干净净的工作区。问题：`.gitignore`文件放哪？答案是放Git仓库根目录下，但其实一个Git仓库也可以有多个`.gitignore`文件，`.gitignore`文件放在哪个目录下，就对哪个目录（包括子目录）起作用。
